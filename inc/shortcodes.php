...
class FWP_Shortcodes {
    ...
    private function register_shortcodes() {
        $shortcodes = array(
            ...
            // Extended shortcodes for page builder
            'cards',
            'icon-box',
            'gallery',
            'gallery-image',
    ...
    public function shortcode_alert($atts, $content = null) {
        ...
        if ($atts['dismissible'] === 'true') {
            ...
            $classes[] = 'fade';
            $classes[] = 'show';
            $dismiss_btn = '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="' . esc_attr__('Sluiten', 'fectionwp-pro') . '"></button>';
        }
    ...
    public function shortcode_tabs($atts, $content = null) {
        $this->tabs_count++;
        $tabs_id = 'fwp-tabs-' . $this->tabs_count;
        ...
        $GLOBALS['fwp_tabs_id'] = $tabs_id;
        $GLOBALS['fwp_tabs_fade'] = $atts['fade'] === 'true';
        $GLOBALS['fwp_tabs_items'] = array();
        $GLOBALS['fwp_tabs_first'] = true;
        ...
        // Build nav tabs
        $nav_items = '';
        foreach ($GLOBALS['fwp_tabs_items'] as $index => $tab) {
        ...
        // Cleanup globals
        unset($GLOBALS['fwp_tabs_id'], $GLOBALS['fwp_tabs_fade'], $GLOBALS['fwp_tabs_items'], $GLOBALS['fwp_tabs_first']);
    ...
    public function shortcode_tab($atts, $content = null) {
        $atts = shortcode_atts(array(
            'title'  => __('Tab', 'fectionwp-pro'),
            'active' => 'false',
            'xclass' => '',
        ), $atts, 'tab');

        if (!isset($GLOBALS['fwp_tabs_id'])) {
            return do_shortcode($content);
        }

        $tabs_id = $GLOBALS['fwp_tabs_id'];
        $tab_index = count($GLOBALS['fwp_tabs_items']);
        $tab_id = $tabs_id . '-' . $tab_index;

        // First tab is active by default
        $is_active = $GLOBALS['fwp_tabs_first'] || $atts['active'] === 'true';
        if ($GLOBALS['fwp_tabs_first']) {
            $GLOBALS['fwp_tabs_first'] = false;
        }
    ...
    public function shortcode_accordion($atts, $content = null) {
        ...
        $accordion_id = 'fwp-accordion-' . $this->accordion_count;
        ...
        $GLOBALS['fwp_accordion_id'] = $accordion_id;
        $GLOBALS['fwp_accordion_always_open'] = $atts['always_open'] === 'true';
        $GLOBALS['fwp_accordion_count'] = 0;
        ...
        unset($GLOBALS['fwp_accordion_id'], $GLOBALS['fwp_accordion_always_open'], $GLOBALS['fwp_accordion_count']);
    ...
    public function shortcode_accordion_item($atts, $content = null) {
        $atts = shortcode_atts(array(
            'title' => __('Item', 'fectionwp-pro'),
            'open'  => 'false',
            'xclass' => '',
        ), $atts, 'accordion-item');

        if (!isset($GLOBALS['fwp_accordion_id'])) {
            return do_shortcode($content);
        }

        $accordion_id = $GLOBALS['fwp_accordion_id'];
        $item_index = $GLOBALS['fwp_accordion_count']++;
        ...
        $parent_attr = '';
        if (!$GLOBALS['fwp_accordion_always_open']) {
    ...
    public function shortcode_modal($atts, $content = null) {
        $this->modal_count++;
        $modal_id = 'fwp-modal-' . $this->modal_count;

        $atts = shortcode_atts(array(
            'title'       => '',
            'button'      => __('Open', 'fectionwp-pro'),
            'button_type' => 'primary',
            'size'        => '',
            'centered'    => 'false',
            'scrollable'  => 'false',
        ...
        if (!empty($atts['title'])) {
            $header = sprintf(
                ...
                esc_html($atts['title']),
                esc_attr__('Sluiten', 'fectionwp-pro')
            );
        }
        ...
        $footer = '';
        if (isset($GLOBALS['fwp_modal_footer'])) {
            $footer = $GLOBALS['fwp_modal_footer'];
            unset($GLOBALS['fwp_modal_footer']);
        }
    ...
    public function shortcode_modal_footer($atts, $content = null) {
        ...
        $GLOBALS['fwp_modal_footer'] = $footer;
        return '';
    }
    ...
    public function shortcode_carousel($atts, $content = null) {
        $this->carousel_count++;
        $carousel_id = 'fwp-carousel-' . $this->carousel_count;
        ...
        $GLOBALS['fwp_carousel_id'] = $carousel_id;
        $GLOBALS['fwp_carousel_items'] = array();
        $GLOBALS['fwp_carousel_first'] = true;
        ...
        // Indicators
        $indicators = '';
        if ($atts['indicators'] === 'true' && count($GLOBALS['fwp_carousel_items']) > 0) {
            $indicator_items = '';
            foreach ($GLOBALS['fwp_carousel_items'] as $index => $item) {
        ...
        if ($atts['controls'] === 'true') {
            $controls = sprintf(
                ...
                esc_attr($carousel_id),
                esc_html__('Vorige', 'fectionwp-pro'),
                esc_html__('Volgende', 'fectionwp-pro')
            );
        }
        ...
        unset($GLOBALS['fwp_carousel_id'], $GLOBALS['fwp_carousel_items'], $GLOBALS['fwp_carousel_first']);
    ...
    public function shortcode_carousel_item($atts, $content = null) {
        ...
        if (!isset($GLOBALS['fwp_carousel_id'])) {
            return '';
        }

        // First item is active by default
        $is_active = $GLOBALS['fwp_carousel_first'] || $atts['active'] === 'true';
        if ($GLOBALS['fwp_carousel_first']) {
            $GLOBALS['fwp_carousel_first'] = false;
        }

        // Register item for indicators
        $GLOBALS['fwp_carousel_items'][] = true;
    ...
    /**
     * Additional shortcodes for metabox integration
     */

    // Cards container shortcode
    public function shortcode_cards($atts, $content = null) {
        $atts = shortcode_atts(array(
            'columns' => '3',
            'style' => 'default',
            'xclass' => '',
        ...
        return sprintf(
            '<div class="row row-cols-1 row-cols-md-%s g-4 %s">%s</div>',
            esc_attr($atts['columns']),
            esc_attr($atts['xclass']),
            do_shortcode($content)
        );
    }
    ...
    public function shortcode_icon_box($atts, $content = null) {
        ...
        $icon_html = sprintf('<i class="bi %s text-%s fs-1 mb-3"></i>', esc_attr($atts['icon']), esc_attr($atts['icon_color']));
        $title_html = !empty($atts['title']) ? '<h5>' . esc_html($atts['title']) . '</h5>' : '';
        $text_html = !empty($atts['text']) ? '<p>' . esc_html($atts['text']) . '</p>' : '';
        
        $content_html = $icon_html . $title_html . $text_html;
        
        if (!empty($atts['url'])) {
            $content_html = sprintf('<a href="%s" class="text-decoration-none">%s</a>', esc_url($atts['url']), $content_html);
        }

        return sprintf('<div class="text-%s %s">%s</div>', esc_attr($atts['align']), esc_attr($atts['xclass']), $content_html);
    }

    // Gallery shortcode
    public function shortcode_gallery($atts, $content = null) {
        $atts = shortcode_atts(array(
            'columns' => '3',
            'lightbox' => 'true',
            'xclass' => '',
        ...
        return sprintf(
            '<div class="row row-cols-1 row-cols-md-%s g-3 %s" data-lightbox="%s">%s</div>',
            esc_attr($atts['columns']),
            esc_attr($atts['xclass']),
            esc_attr($atts['lightbox']),
            do_shortcode($content)
        );
    }

    // Gallery image shortcode
    public function shortcode_gallery_image($atts, $content = null) {
        $atts = shortcode_atts(array(
            'src' => '',
            'alt' => '',
        ), $atts, 'gallery-image');
        ...
        return sprintf(
            '<div class="col"><img src="%s" alt="%s" class="img-fluid rounded"></div>',
            esc_url($atts['src']),
            esc_attr($atts['alt'])
        );
    }
    ...
    // Timeline shortcode
    public function shortcode_timeline($atts, $content = null) {
        $atts = shortcode_atts(array('xclass' => ''), $atts, 'timeline');
        return sprintf('<div class="fwp-timeline %s">%s</div>', esc_attr($atts['xclass']), do_shortcode($content));
    }

    // Timeline item shortcode
    public function shortcode_timeline_item($atts, $content = null) {
        $atts = shortcode_atts(array(
            'date' => '',
            'title' => '',
        ), $atts, 'timeline-item');
        ...
        return sprintf(
            '<div class="timeline-item mb-4"><div class="timeline-marker"></div><div class="timeline-content"><h6 class="text-muted mb-1">%s</h6><h5>%s</h5><p>%s</p></div></div>',
            esc_html($atts['date']),
            esc_html($atts['title']),
            do_shortcode($content)
        );
    ...
    // Counter shortcode (animated number)
    public function shortcode_counter($atts, $content = null) {
        ...
        return sprintf(
            '<div class="fwp-counter text-center %s"><div class="counter-value display-4 fw-bold" data-count="%s" data-duration="%s">%s%s%s</div>%s</div>',
            esc_attr($atts['xclass']),
            esc_attr($atts['value']),
            ...
            !empty($atts['label']) ? '<p class="counter-label text-muted">' . esc_html($atts['label']) . '</p>' : ''
        );
    }

    // Product card shortcode
    public function shortcode_product_card($atts, $content = null) {
        $atts = shortcode_atts(array(
            'image' => '',
            'title' => '',
            'description' => '',
        ...
        $badge_html = '';
        if (!empty($atts['badge'])) {
            $badge_classes = array('sale' => 'danger', 'new' => 'success', 'hot' => 'warning', 'limited' => 'info');
            $badge_class = isset($badge_classes[$atts['badge']]) ? $badge_classes[$atts['badge']] : 'primary';
            $badge_html = sprintf('<span class="badge bg-%s position-absolute top-0 end-0 m-2">%s</span>', $badge_class, esc_html(ucfirst($atts['badge'])));
        }

        return sprintf(
            '<div class="card h-100 %s">
                <div class="position-relative">%s%s</div>
                <div class="card-body">
                    <h5 class="card-title">%s</h5>
                    <p class="card-text">%s</p>
                    <div class="d-flex align-items-center justify-content-between">
                        <div class="price">%s<span class="h5 mb-0 text-primary">%s</span></div>
                        <a href="%s" class="btn btn-primary">%s</a>
                    </div>
                </div>
            </div>',
            esc_attr($atts['xclass']),
            !empty($atts['image']) ? sprintf('<img src="%s" class="card-img-top" alt="%s">', esc_url($atts['image']), esc_attr($atts['title'])) : '',
            $badge_html,
            esc_html($atts['title']),
            esc_html($atts['description']),
            !empty($atts['old_price']) ? sprintf('<del class="text-muted me-2">%s</del>', esc_html($atts['old_price'])) : '',
            esc_html($atts['price']),
    ...
    public function shortcode_products_grid($atts, $content = null) {
        $atts = shortcode_atts(array(
            'title' => '',
            'columns' => '3',
            'xclass' => '',
        ...
        return sprintf(
            '<section class="fwp-products-grid %s">%s<div class="row row-cols-1 row-cols-md-%s g-4">%s</div></section>',
            esc_attr($atts['xclass']),
            !empty($atts['title']) ? '<h2 class="mb-4">' . esc_html($atts['title']) . '</h2>' : '',
            esc_attr($atts['columns']),
            do_shortcode($content)
        );
    }
    ...
    public function shortcode_contact_form($atts, $content = null) {
        $atts = shortcode_atts(array(
            ...
            'success_msg' => 'Bedankt voor uw bericht!',
            'btn_text' => 'Verstuur',
            'fields' => 'simple',
            'xclass' => '',
        ), $atts, 'contact-form');
        ...
        $form_id = 'fwp-contact-form-' . uniqid();
        
        return sprintf(
            '<div class="fwp-contact-form %s">
                <h3>%s</h3>
                <form id="%s" class="needs-validation" novalidate>
                    <div class="mb-3"><input type="text" class="form-control" name="name" placeholder="Naam" required></div>
                    <div class="mb-3"><input type="email" class="form-control" name="email" placeholder="E-mail" required></div>
                    %s
                    <div class="mb-3"><textarea class="form-control" name="message" rows="5" placeholder="Bericht" required></textarea></div>
            ...
            esc_html($atts['title']),
            esc_attr($form_id),
            $atts['fields'] === 'extended' ? '<div class="mb-3"><input type="tel" class="form-control" name="phone" placeholder="Telefoon"></div><div class="mb-3"><input type="text" class="form-control" name="subject" placeholder="Onderwerp"></div>' : '',
            esc_html($atts['btn_text'])
        );
    }

    // Newsletter shortcode
    public function shortcode_newsletter($atts, $content = null) {
        $atts = shortcode_atts(array(
            'title' => 'Schrijf je in voor onze nieuwsbrief',
            'description' => '',
            'placeholder' => 'Uw e-mailadres',
            'btn_text' => 'Inschrijven',
            'style' => 'inline',
            'bg_color' => '',
            'xclass' => '',
        ), $atts, 'newsletter');

        $bg_class = !empty($atts['bg_color']) ? 'bg-' . esc_attr($atts['bg_color']) : '';
        $form_class = $atts['style'] === 'inline' ? 'input-group' : '';
        $input_class = $atts['style'] === 'inline' ? '' : 'mb-3';

        return sprintf(
            '<div class="fwp-newsletter %s %s p-4 rounded">
                <div class="text-center mb-3"><h4>%s</h4>%s</div>
                <form class="%s">
                    <input type="email" class="form-control %s" placeholder="%s" required>
            ...
            esc_attr($atts['xclass']),
            esc_html($atts['title']),
            !empty($atts['description']) ? '<p class="text-muted">' . esc_html($atts['description']) . '</p>' : '',
            esc_attr($form_class),
            esc_attr($input_class),
            esc_attr($atts['placeholder']),
            $atts['style'] === 'inline' ? '' : 'w-100',
            esc_html($atts['btn_text'])
        );
    }
    ...
    public function shortcode_social_links($atts, $content = null) {
        ...
        $size_class = $atts['size'] === 'lg' ? 'fs-3' : ($atts['size'] === 'sm' ? 'fs-6' : 'fs-5');
        $html = '<div class="fwp-social-links d-flex gap-3 ' . esc_attr($atts['xclass']) . '">';
        
        foreach ($networks as $network => $icon) {
            if (!empty($atts[$network])) {
                $html .= sprintf(
                    '<a href="%s" class="text-decoration-none %s" target="_blank" rel="noopener noreferrer"><i class="bi %s"></i></a>',
                    esc_url($atts[$network]),
                    esc_attr($size_class),
                    esc_attr($icon)
                );
            }
    ...
    public function shortcode_map($atts, $content = null) {
        ...
        $map_url = sprintf(
            'https://maps.google.com/maps?q=%s&z=%s&output=embed',
            urlencode($atts['location']),
            esc_attr($atts['zoom'])
        );

        return sprintf(
            '<div class="fwp-map ratio ratio-16x9 %s" style="height: %spx;"><iframe src="%s" style="border:0;" allowfullscreen="" loading="lazy"></iframe></div>',
            esc_attr($atts['xclass']),
 ...
 */
function fwp_init_shortcodes() {
    FWP_Shortcodes::get_instance();
}
add_action('init', 'fwp_init_shortcodes');

/**
 * Initialiseer tooltips en popovers via JavaScript
 */
function fwp_shortcodes_scripts() {
    global $post;
...
add_action('wp_enqueue_scripts', 'fwp_shortcodes_scripts', 20);
