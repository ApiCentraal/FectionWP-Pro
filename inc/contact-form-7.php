 ...
 */
function fwp_cf7_enqueue_styles() {
    wp_enqueue_style(
        'fwp-cf7-bootstrap',
...
add_action('wp_enqueue_scripts', 'fwp_cf7_enqueue_styles', 20);

/**
 * Voeg Bootstrap classes toe aan CF7 form wrapper
 */
function fwp_cf7_form_class_attr($class) {
 ...
 */
function fwp_cf7_ajax_loader() {
    return '<span class="spinner-border spinner-border-sm ms-2" role="status"><span class="visually-hidden">' . esc_html__('Laden...', 'fectionwp-pro') . '</span></span>';
}
add_filter('wpcf7_ajax_loader', 'fwp_cf7_ajax_loader');

/**
 * Voeg novalidate attribuut toe voor custom validation
 */
function fwp_cf7_form_novalidate($form) {
    $form = str_replace('<form ', '<form novalidate ', $form);
    return $form;
}
add_filter('wpcf7_form_elements', 'fwp_cf7_form_novalidate');
