...
function fwp_elementor_support() {
    ...
    add_action('elementor/frontend/section/before_render', function($element) {
        ...
        if (isset($settings['fwp_bootstrap_grid']) && $settings['fwp_bootstrap_grid'] === 'yes') {
...
function fwp_divi_support() {
    ...
    add_filter('et_pb_section_content_width', function() {
    ...
    add_filter('et_builder_module_wrapper_attributes', function($attrs, $props) {
        // Voeg Bootstrap classes toe indien gewenst
        if (isset($props['use_bootstrap_grid']) && $props['use_bootstrap_grid'] === 'on') {
...
function fwp_sync_divi_colors() {
    ...
    update_option('et_divi', array_merge(
...
function fwp_beaver_builder_support() {
    ...
    // Synchroniseer container breedte
    add_filter('fl_builder_content_width', function() {
        return '1320'; // Bootstrap container-xxl
    });

    // Voeg Bootstrap klassen toe aan Beaver Builder rijen
    add_filter('fl_builder_row_custom_class', function($class, $settings) {
        if (isset($settings->use_bootstrap_grid) && $settings->use_bootstrap_grid === 'yes') {
            $class .= ' container';
        }
        return $class;
    }, 10, 2);
...
function fwp_wpbakery_support() {
    ...
    add_filter('vc_shortcodes_css_class', function($class, $tag) {
        if ($tag === 'vc_row') {
            $class .= ' container';
        }
        return $class;
...
function fwp_oxygen_support() {
    ...
    // Synchroniseer container breedte
    add_filter('oxygen_default_content_width', function() {
        return '1320';
    });
...
function fwp_brizy_support() {
    ...
    // Synchroniseer container breedte
    add_filter('brizy_container_max_width', function() {
        return 1320;
    });

    // Voeg Bootstrap CSS variabelen toe
    add_action('brizy_template_compile_page', function() {
        $primary = get_theme_mod('fwp_primary_color', '#0d6efd');
...
function fwp_thrive_architect_support() {
    ...
    // Synchroniseer container breedte
    add_filter('tcb_content_width', function() {
        return '1320';
    });

    // Bootstrap kleur integratie
    add_filter('tcb_main_style_family_config', function($config) {
        $config['colors'] = array(
            'primary' => get_theme_mod('fwp_primary_color', '#0d6efd'),
            'secondary' => get_theme_mod('fwp_secondary_color', '#6c757d'),
            'success' => get_theme_mod('fwp_success_color', '#198754'),
...
function fwp_disable_theme_layout_for_builders() {
    if (fwp_is_page_builder_used() && is_singular()) {
        ...
        add_filter('fwp_container_type', function() {
...
// Log succesvolle page builder integratie
if (defined('WP_DEBUG') && WP_DEBUG && fwp_is_page_builder_active()) {
    error_log('FectionWP Pro: Page builder ondersteuning actief');
}
